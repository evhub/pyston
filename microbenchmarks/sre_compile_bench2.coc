...
patterns = [
    ...
    '((?:<p>(?:\\&middot\\;|\\*|\\\u2022|\\&\\#149\\;|\\&bull\\;|\\&\\#8226\\;).*?[a-zA-Z].*?</p>\\s*)+)',
    '(?:<p>(?:&nbsp;|\\s|<br \\/>)*?</p>\\s*)+\\Z',
    ...
    '\\?|[-+]?[.\\w]+$',
    '(?:W/)?"((?:\\\\.|[^"])*)"',
    '^\\w{3}, (?P<day>\\d{2}) (?P<mon>\\w{3}) (?P<year>\\d{4}) (?P<hour>\\d{2}):(?P<min>\\d{2}):(?P<sec>\\d{2}) GMT$',
    '^\\w{6,9}, (?P<day>\\d{2})-(?P<mon>\\w{3})-(?P<year>\\d{2}) (?P<hour>\\d{2}):(?P<min>\\d{2}):(?P<sec>\\d{2}) GMT$',
    '^\\w{3} (?P<mon>\\w{3}) (?P<day>[ \\d]\\d) (?P<hour>\\d{2}):(?P<min>\\d{2}):(?P<sec>\\d{2}) (?P<year>\\d{4})$',
    '\\s*,\\s*',
    '^From ',
    '[ \\(\\)<>@,;:\\\\"/\\[\\]\\?=]',
    '(\\{\\%.*?\\%\\}|\\{\\{.*?\\}\\}|\\{\\#.*?\\#\\})',
